...
def resampling(svs,weights):
    ...
    while(len(resampled_particle)<N):
        for sp in sorted_particle:
            ...
            tmp = tmp.tolist()
            # print type(tmp*[sp[0]])
            a = [tmp*x for x in sp[0]]
            # print a
            resampled_particle += a
...
def filtration(svs,mv,systemModel,likelihood):
    #システムモデルに用いる乱数を生成
    if type(svs[1]) is np.float64 : return
    dim = len(svs[1])
    print dim
    N = len(svs)
    sigma = 2.0 #スケール？
...
def showImage(svs,img):
    ...
    for sv in svs:
        ...
        # print sv
        # print type(sv[0])
        if type(sv[0]) is np.float64 : break
        #!!!パーティクル位置をintに変換している
        cv.Circle(dst,(int(sv[0][0]),int(sv[0][1])),3,cv.CV_RGB(0,0,255))
    #表示
